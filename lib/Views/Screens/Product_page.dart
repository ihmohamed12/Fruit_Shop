      ...
      child: Scaffold(
        ...
        bottomNavigationBar: Padding(
          ...
          child: ElevatedButton(
              onPressed: () {
                if (controller.bought.isEmpty) {
                  controller.bought
                      .add(
                      Product(
                      name: controller.current.name,
                      type: controller.current.type,
                      price: controller.current.price,
                      description: controller.current.description,
                      imagePath: controller.current.imagePath,
                      calories: controller.current.calories,
                      weight: controller.current.weight,
                      rating: controller.current.rating,
                      weightBought: controller.current.weightBought));


                } else {
                  bool newItem = true;

                  for (int i = 0; i < controller.bought.length; i++) {
                    if(controller.bought[i].name ==controller.current.name ){
                      newItem=false;
                      controller.bought[i].weightBought+=controller.current.weightBought;
                      break;
                    }
                  }
                  if(newItem){
                    controller.bought.add( Product(
                        name: controller.current.name,
                        type: controller.current.type,
                        price: controller.current.price,
                        description: controller.current.description,
